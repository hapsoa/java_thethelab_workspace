public class Vector2D implements Cloneable {
    ...
    public void setVector2D(Vector2D vector2D){ setVector2D(vector2D.x, vector2D.y); }

    public void setVector2D(float value){
        setX(value);
        setY(value);
    }
    ...
    public static Vector2D getMultiplied(Vector2D vector2D, float value) {
        Vector2D multiplied = vector2D.clone();
    ...
    public static Vector2D getMultiplied(Vector2D v1, Vector2D v2) {
        ...
        multiplied.multiply(v2);
        return multiplied;
    }

    public void divide(float x, float y){

        if(x == 0 || y == 0) throw new IllegalArgumentException("Divide Zero");
        this.x /= x;
        this.y /= y;
    }

    public void divide(Vector2D vector2D){
        this.divide(vector2D.x, vector2D.y);
    }


    public static Vector2D getDivided(Vector2D v1, Vector2D v2){
        Vector2D divided = v1.clone();
        divided.divide(v2);
        return divided;
    }
     ...
     */
    float magnitude() { return getDistance(this, new Vector2D(0, 0)); }
    ...
    /**
     * rotates Vector2D based on a Vector2D(base) to the extent of (radian).
     * @param radian parameter for rotating.
     * @param base  parameter for rotating.
     */
    public void rotate(float radian, Vector2D base) {

        Vector2D tmpVector = this.clone();
        tmpVector.subtract(base);
        tmpVector.x = (float) (Math.cos(radian) * x - Math.sin(radian) * y);
        tmpVector.y = (float) (Math.sin(radian) * x + Math.cos(radian) * y);
        tmpVector.add(base);
        setVector2D(tmpVector);
    }

    /**
     * rotates Vector2D based on a Vector2D(0, 0) to the extent of (radian).
     * @param radian parameter for rotating.
     */
    public void rotateFromOrigin(float radian) { rotate(radian, new Vector2D(0, 0)); }

    /**
     * returns new Vector2D after rotation with three parameters -Vector2D, radian, and Vector2D(base)-.
     * @param vector2D parameter which will be rotated.
     * @param radian    parameter used as the extent for rotating.
     * @param base     parameter which will be used as a standard.
     * @return new Vector2D after rotation.
     */
    public static Vector2D getRotated(Vector2D vector2D, float radian, Vector2D base) {
        Vector2D rotated = getSubtracted(vector2D, base).clone();
        rotated.rotate(radian, base);
        return rotated;
    }

    /**
     * returns new Vector2D after rotation with two parameters -Vector2D, radian- based on a Vector2D(0, 0).
     * @param vector2D parameter which will be rotated.
     * @param radian    parameter used as the extent for rotating.
     * @return new Vector2D after rotation.
     */
    public static Vector2D getRotatedFromOrigin(Vector2D vector2D, float radian) {
        Vector2D rotated = vector2D.clone();
        rotated.rotateFromOrigin(radian);
        return rotated;
    }
    /**
     * clones Vector2D
     ...
     */
    void print(){ System.out.print("X: " + x + "Y:" + y); }

    /**
     * prints Vector 2D state
     */
    void println(){ System.out.println("X: " + x + "Y:" + y); }
}