...
public class Block extends View implements Constants {
    ...
    Block(Arkanoid pApplet) {
        ...
        size = new Vector2(BLOCK_SIZE_X, BLOCK_SIZE_Y);
    }

    public void setIndex(int index, int count) {
        this.index = index;
        position = new Vector2(Util.getPosXByIndex(index, count), Util.getPosYByIndex(index, count));
        position.x += 45;
    }
    ...
    public void makeItem(List<Item> items) {
        ...
        if (item > 0) {
            Item I = new Item(pApplet, item, position.x, position.y);
            items.add(I);
    ...
    public void render() {
        ...
        if (visible) {
            ...
            if (color == BLOCK_WHITE)
                pApplet.image(blockimages.get(0), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);
            else if (color == BLOCK_ORANGE)
                pApplet.image(blockimages.get(1), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);
            else if (color == BLOCK_SKYBLUE)
                pApplet.image(blockimages.get(2), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);

            else if (color == BLOCK_GREEN)
                pApplet.image(blockimages.get(3), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);

            else if (color == BLOCK_RED)
                pApplet.image(blockimages.get(4), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);

            else if (color == BLOCK_BLUE)
                pApplet.image(blockimages.get(5), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);

            else if (color == BLOCK_PINK)
                pApplet.image(blockimages.get(6), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);

            else if (color == BLOCK_YELLOW)
                pApplet.image(blockimages.get(7), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);
            else if (color == BLOCK_HARD)
                pApplet.image(blockimages.get(8), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);
            else if (color == BLOCK_IMMORTAL)
                pApplet.image(blockimages.get(9), position.x, position.y, BLOCK_SIZE_X, BLOCK_SIZE_Y);


            pApplet.fill(0);
            pApplet.text(item, (int) position.x + 20, (int) position.y + 20, 20);
//
