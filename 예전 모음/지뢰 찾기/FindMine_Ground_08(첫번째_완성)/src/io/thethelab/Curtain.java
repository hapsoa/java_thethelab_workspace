...
public class Curtain extends View {
    ...
    private Block[][] curtain;
    private boolean isFailed = false;
    private int numOfRightFlag;
    private boolean isSuccess = false;

    Curtain(Window pApplet, int sizeX, int sizeY) {
        super(pApplet);
    ...
    public void update() {
        ...
        for (int i = 0; i < curtain.length; i++)
            for (int j = 0; j < curtain[0].length; j++) {
                ...
                // 모두 깃발 표시 시
                if (pApplet.map.map[i][j].property == Constant.BOMB &&
                        curtain[i][j].property == Constant.FLAG) {
                    numOfRightFlag++;
                    if (numOfRightFlag == pApplet.numOfBomb) {
    ...
    private void delete(int i, int j, Map map) {

        if (map.map[i][j].property != Constant.BOMB) {
            curtain[i][j].property = Constant.SHOWN;
            if (map.map[i][j].property == Constant.NO_BOMB)
                openAround(i, j, map);
        } else {
            isFailed = true;
        }
    ...
    private void openAround(int i, int j, Map map) {
        if (i - 1 >= 0 && j - 1 >= 0 &&
                curtain[i - 1][j - 1].property == Constant.HIDDEN) {
            curtain[i - 1][j - 1].property = Constant.SHOWN;
            if (map.map[i - 1][j - 1].property == Constant.NO_BOMB)
                openAround(i - 1, j - 1, map);
        }
        if (i - 1 >= 0 &&
                curtain[i - 1][j].property == Constant.HIDDEN) {
            curtain[i - 1][j].property = Constant.SHOWN;
            if (map.map[i - 1][j].property == Constant.NO_BOMB)
                openAround(i - 1, j, map);
        }
        if (i - 1 >= 0 && j + 1 < curtain[0].length &&
                curtain[i - 1][j + 1].property == Constant.HIDDEN) {
            curtain[i - 1][j + 1].property = Constant.SHOWN;
        ...
        if (j - 1 >= 0 &&
            ...
            if (map.map[i][j - 1].property == Constant.NO_BOMB)
                openAround(i, j - 1, map);
        }
        if (j + 1 < curtain[0].length &&
                curtain[i][j + 1].property == Constant.HIDDEN) {
            curtain[i][j + 1].property = Constant.SHOWN;
