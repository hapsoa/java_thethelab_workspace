...
//    public static HashMap<Integer, ArrayList<OnCollisionListener>> onCollisionListeners
//            = new HashMap<>();
//
...
//
//    public void setCollisionListener(View view1, View view2, OnCollisionListener listener){
//        this.onCollisionListener = listener;
//    }


    public static HashMap<Integer, ArrayList<View>> collisionViews = new HashMap<>();
    public static List<View> bin = new ArrayList<>();
    ...
    public static void update() {
                ...
                if (!i.equals(j)) {
                    for (int k = 0; k < srcs.size(); k++) {
                        for (int m = 0; m < dsts.size(); m++) {
                            ...
                            if(src.shape == View.RECT && dst.shape == View.CIRCLE)
                                collisionRectCircleOutside(src, dst);
                            else if(src.shape == View.CIRCLE && dst.shape == View.RECT)
                                collisionRectCircleOutside(dst, src);
                            else if(src.shape == View.RECT_INSIDE && dst.shape == View.CIRCLE)
                                collisionRectCircleInside(src, dst);
                            else if(src.shape == View.CIRCLE && dst.shape == View.RECT_INSIDE)
                                collisionRectCircleInside(dst, src);
                            else if(src.shape == View.RECT_OUTSIDE && dst.shape == View.RECT_OUTSIDE)
        ...
        for(Integer i : keySet){
            collisionViews.get(i).removeAll(bin);
        }
...
//        Vector2 rect2pos = rect2.position;
        ...
        float rectPos2X = rect2.position.x - rect2.size.x/2;
        float rectPos2Y = rect2.position.y - rect2.size.y/2;
