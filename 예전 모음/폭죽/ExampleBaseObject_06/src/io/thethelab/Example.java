...
//    List<ExamplePoint> points;
    List<Rect> rects;
    List<Circle> circles;
    ArrayList<ArrayList<Circle>> circlesList;
...
//        }
        ...
        for (int i = 0; i < rects.size(); i++) {
            if (Math.abs(rects.get(i).vx) <= 0.1f) {
                rects.get(i).destroy(circlesList);
                rects.remove(i);
            }
        }
        ...
        for (int i = 0; i < circlesList.size(); i++) {
            for (int j = 0; j < circlesList.get(i).size(); j++) {
                circlesList.get(i).get(j).update();
                circlesList.get(i).get(j).render();
            }
        }

        //폭발한 폭죽 정보를 삭제한다.
        if (circlesList.size() > 30)
            circlesList.remove(0);
