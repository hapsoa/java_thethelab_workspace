...
// ** 점을 찍고 그 점이 움직여야 함.
    class Point {
        ...
        Point() {
            this.x = (int) (mouseX + Math.random() * 20 - 10);
            this.y = (int) (mouseY + Math.random() * 20 - 10);
            this.raduis = (float) (Math.random() * 10 + 5);

            this.r = (255 + tick) % 255 + (int) (Math.random() * 5);
            this.g = 255;
            this.b = 255;

            this.deltaAccelx = (float) (Math.random() * 0.1f) + -0.05f;
            this.deltaAccely = (float) (Math.random() * 0.1f) + -0.05f;
        }
...
//                if (onePoint.get(onePoint.size()-1).x> 960 || onePoint.get(onePoint.size()).x < 0 ||
//                        onePoint.get(onePoint.size()).y > 640 || onePoint.get(onePoint.size()).y < 0)
//                    points.remove(j);
//            }
